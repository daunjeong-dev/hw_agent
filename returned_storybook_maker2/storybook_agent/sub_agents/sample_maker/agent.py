...
def gate_sample_generation(
    ...
    filename = tool_context.state.get("sample_generation_filename")
    ...
    if status == "PENDING_REVIEW" and last_invocation_id == tool_context.invocation_id:
        return {
            "status": "done",
            "filename": filename,
            "message": f"이미 생성된 샘플이 있습니다 ({filename}). 이 결과로 최종 출력을 작성하세요.",
        }
...
class IllustratorConceptOutput(BaseModel):
    ...
    sample_image_name: str = Field(description="generated sample image name")

_sample_maker_before, _sample_maker_after = make_agent_lifecycle_logger(
    "샘플 이미지 생성 시작", "샘플 이미지 생성 완료"
)
...
sample_maker_agent = Agent(
    ...
    before_agent_callback=_sample_maker_before,
    after_agent_callback=_sample_maker_after,
    before_tool_callback=[gate_sample_generation, log_tool_start],
